...
const PERMISSIONS_LIST: Permission[] = [
  ...
  },
  {
    ...
    defaultRoles: [UserRole.Admin, UserRole.Manager, UserRole.Consultant],
  },
  {
    id: "lead_assign",
    module: "Quản lý Lead",
...
export default function RolesPage() {
  ...
  const togglePermission = (permId: string, role: UserRole) => {
    ...
    if (role === UserRole.Admin && (permId === "user_manage" || permId === "sys_settings")) {
      addToast({
        type: "warning",
  ...
  const roles: UserRole[] = [UserRole.Admin, UserRole.Manager, UserRole.Consultant];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-navy-400 mt-0.5">
            Quản lý quyền hạn chi tiết cho từng vai trò 
          </p>
        </div>
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Vai trò {role === UserRole.Admin ? "hệ thống tối cao" : role === UserRole.Manager ? "quản lý vận hành" : "tiếp nhận & xử lý"}
                </p>
              </div>
              <Badge variant={role === UserRole.Admin ? "destructive" : role === UserRole.Manager ? "warning" : "success"}>
                {role}
              </Badge>
            </CardContent>
          </Card>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-navy-700/50 bg-navy-800/40">
                  <th className="text-left px-6 py-4 text-xs font-bold text-navy-300 uppercase tracking-wider min-w-[280px]">
                    Quyền hạn / Chức năng
                  </th>
                  {roles.map((role) => (
                    <th
              ...
              <tbody className="divide-y divide-navy-700/30">
                {modules.map((mod) => (
                  <React.Fragment key={mod}>
                    {/* Module Header */}
                    <tr className="bg-muted/60 dark:bg-navy-800/20">
                      <td
                        colSpan={roles.length + 1}
                        className="px-6 py-2.5 text-xs font-bold text-primary uppercase tracking-wider bg-muted/80 dark:bg-navy-800/30"
                      >
                        {mod}
                      </td>
                    </tr>
                    ...
                    {PERMISSIONS_LIST.filter((p) => p.module === mod).map((perm) => (
                      ...
                      >
                        ...
                        {roles.map((role) => {
                          ...
                          const isLocked = role === UserRole.Admin && (perm.id === "user_manage" || perm.id === "sys_settings");
                                ...
                                className={`inline-flex items-center justify-center h-8 w-8 rounded-lg transition-all duration-200 ${
                                  isChecked
                                    ? isLocked
                                      ...
                                      : "bg-primary text-navy-950 shadow-md shadow-primary/20 hover:bg-primary/80"
                                    : "bg-navy-800/50 text-navy-600 border border-navy-700/50 hover:border-primary hover:text-primary"
                                }`}

...
apiClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    if (typeof window !== "undefined") {
      const token = getTokenFromMemory() || localStorage.getItem("access_token");
...
apiClient.interceptors.response.use(
  ...
  (error: AxiosError<ApiError>) => {
    ...
    const isLoginRequest = error.config?.url?.includes("/api/Auth/login") || error.config?.url?.includes("/login");
